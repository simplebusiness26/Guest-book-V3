...
export default function Notifications(){
  ...
  const counts=useMemo(()=>{
    const result={all:items.length,social:0,clubs:0,account:0};
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh}/>}> 
      <View style={styles.headingRow}>
        <View style={styles.headingText}>
          ...
          <Text style={styles.subtitle}>Social activity, club updates and account alerts in one place.</Text>
        </View>
      ...
      {!loading && filteredItems.length===0 && <View style={styles.emptyCard}><Text style={styles.emptyIcon}>{activeCategory==="social"?"✨":"🔔"}</Text><Text style={styles.emptyTitle}>No {activeCategory==="all"?"notifications":activeCategory+" notifications"} yet</Text><Text style={styles.emptyText}>New updates will appear here.</Text></View>}
      ...
      {!loading && filteredItems.map(item=>{
        ...
        return(
          ...
          >
            <View style={[styles.iconWrap,category==="social"&&styles.socialIconWrap,!pastUpdate&&!item.read_at&&styles.unreadIconWrap]}><Text style={styles.icon}>{notificationIcon(item)}</Text></View>
            <View style={styles.cardText}>
              ...
              <Text style={[styles.categoryLabel,category==="social"&&styles.socialCategory]}>{category}</Text>
              {!!label && <Text style={[
                styles.statusBadge,
...
const styles=StyleSheet.create({
  ...
  socialCard:{backgroundColor:"#faf8ff",borderColor:"#d9cff6"},accountCard:{backgroundColor:"#fbfcfe"},
  unreadCard:{backgroundColor:"#eef3ff",borderColor:"#aec2f2"},socialUnreadCard:{backgroundColor:"#f1ecff",borderColor:"#bca8f1"},
